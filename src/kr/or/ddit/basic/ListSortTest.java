...
// 졍렬 방식을 정해주는 Class를 작성한다. (외부 정렬 기준 클래스라고 한다)
...
class Desc implements Comparator<String>{
	...
	// 예) 오름차순 일 경우 ==> 앞의 값이 크면 양수, 같으면 0, 뒤의 값이 크면 음수를 반환하도록 하면 된다.
	
	// String객체에는 정렬을 위해서 compareTo()메서드가 구현되어 있는데 
	// 이 메서드의 반환값은 오름차순에 맞게 반환되도록 구현되어 있다
	// 형식) 앞문자열.compareTo(뒤문자열);
	
	// (Wrapper클래스와 Date, File클래스도에도 구현되어 있다)

	@Override
	public int compare(String str1, String str2) { //두개의 데이터를 가져와서 비교한다 첫번째 변수는 앞에있는 데이터 뒤에 있는 데이터 뒤에있는 데이터
